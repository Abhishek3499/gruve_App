...
class PerformanceTestSuite {
  static final PerformanceTestSuite _instance =
  ...
  void startPerformanceTesting() {
    developer.log(
      '🧪 [PERF TEST] Starting comprehensive performance test suite',
    ...
    WidgetsBinding.instance.addTimingsCallback((timings) {
      for (final timing in timings) {
        ...
        if (frameTime > 16666) {
          ...
          _droppedFrames++;
          developer.log(
            '⚠️ [PERF TEST] Frame drop detected: $frameTime μs',
            name: 'FrameTest',
          );
        }
  ...
  void _performStressTest() {
    ...
    if (stressTime.inMilliseconds > 100) {
      developer.log(
        '⚠️ [PERF TEST] Stress test took ${stressTime.inMilliseconds}ms',
        name: 'StressTest',
      );
    }
  }
  ...
  void _simulateMemoryAllocation() {
    ...
    final largeList = List.generate(
      1000,
      (index) => 'Item $index - ${DateTime.now().millisecondsSinceEpoch}',
    );
  ...
  void _checkMemoryUsage() {
    ...
    developer.log(
      '💾 [PERF TEST] Memory check at ${DateTime.now()}',
      name: 'MemoryTest',
    );
  }
  ...
  void testImageLoadingPerformance(List<String> imageUrls) async {
    developer.log(
      '🖼️ [PERF TEST] Testing image loading performance',
      name: 'ImageTest',
    );
    ...
    for (final imageUrl in imageUrls) {
      ...
      try {
        ...
        await Future.delayed(
          Duration(milliseconds: 200 + (imageUrl.hashCode % 800)),
        );
        ...
        developer.log(
          '🖼️ [PERF TEST] Image loaded in ${loadTime.inMilliseconds}ms: $imageUrl',
          name: 'ImageTest',
        );

        if (loadTime.inMilliseconds > 1000) {
          developer.log(
            '⚠️ [PERF TEST] Slow image load: ${loadTime.inMilliseconds}ms',
            name: 'ImageTest',
          );
        }
      } catch (e) {
  ...
  void testSocketPerformance() {
    developer.log(
      '🔌 [PERF TEST] Testing socket performance',
      name: 'SocketTest',
    );
    ...
    developer.log(
      '🔌 [PERF TEST] Socket test completed in ${socketTestTime.inMilliseconds}ms',
      name: 'SocketTest',
    );
  }
  ...
  void _simulateSocketOperations() async {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      developer.log(
        '🔌 [PERF TEST] Socket event $i simulated',
        name: 'SocketTest',
      );
    }
  }
  ...
  void testScrollingPerformance() {
    developer.log(
      '📜 [PERF TEST] Testing scrolling performance',
      name: 'ScrollTest',
    );
    ...
    developer.log(
      '📜 [PERF TEST] Scroll test completed in ${scrollTestTime.inMilliseconds}ms',
      name: 'ScrollTest',
    );
  }
  ...
  void generateTestReport() {
    if (_testStartTime == null) {
      developer.log(
        '❌ [PERF TEST] No test data available',
        name: 'PerformanceTestSuite',
      );
      return;
    }
    ...
    developer.log(
      '📊 [PERF TEST] ========== PERFORMANCE TEST REPORT ==========',
      name: 'PerformanceTestSuite',
    );
    developer.log(
      '⏱️ [PERF TEST] Total test duration: ${totalTestTime.inSeconds}s',
      name: 'PerformanceTestSuite',
    );

    // Frame performance analysis
    if (_testResults['frame_times'] != null &&
        _testResults['frame_times']!.isNotEmpty) {
      ...
      developer.log(
        '  Drop rate: ${dropRate.toStringAsFixed(1)}%',
        name: 'PerformanceTestSuite',
      );
      developer.log(
    ...
    if (_testResults['stress_test_times'] != null &&
        _testResults['stress_test_times']!.isNotEmpty) {
      final stressTimes = _testResults['stress_test_times']!;
      final avgStressTime =
          stressTimes.reduce((a, b) => a + b) / stressTimes.length;
      final maxStressTime = stressTimes.reduce((a, b) => a > b ? a : b);

      developer.log(
        '💪 [PERF TEST] Stress Test Performance:',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Average operation time: ${avgStressTime.toStringAsFixed(1)}ms',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Max operation time: ${maxStressTime}ms',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Total operations: ${stressTimes.length}',
        name: 'PerformanceTestSuite',
      );
    }

    // Image loading analysis
    if (_testResults['image_load_times'] != null &&
        _testResults['image_load_times']!.isNotEmpty) {
      final imageTimes = _testResults['image_load_times']!;
      final avgImageTime =
          imageTimes.reduce((a, b) => a + b) / imageTimes.length;
      final slowImages = imageTimes.where((time) => time > 1000).length;

      developer.log(
        '🖼️ [PERF TEST] Image Loading Performance:',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Average load time: ${avgImageTime.toStringAsFixed(0)}ms',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Slow loads (>1s): $slowImages/${imageTimes.length}',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Total images: ${imageTimes.length}',
        name: 'PerformanceTestSuite',
      );
    }

    // Socket performance analysis
    if (_testResults['socket_test_times'] != null &&
        _testResults['socket_test_times']!.isNotEmpty) {
      final socketTimes = _testResults['socket_test_times']!;
      final avgSocketTime =
          socketTimes.reduce((a, b) => a + b) / socketTimes.length;

      developer.log(
        '🔌 [PERF TEST] Socket Performance:',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Average operation time: ${avgSocketTime.toStringAsFixed(1)}ms',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Total operations: ${socketTimes.length}',
        name: 'PerformanceTestSuite',
      );
    }

    // Scrolling performance analysis
    if (_testResults['scroll_test_times'] != null &&
        _testResults['scroll_test_times']!.isNotEmpty) {
      final scrollTimes = _testResults['scroll_test_times']!;
      final avgScrollTime =
          scrollTimes.reduce((a, b) => a + b) / scrollTimes.length;

      developer.log(
        '📜 [PERF TEST] Scrolling Performance:',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Average scroll operation: ${avgScrollTime.toStringAsFixed(1)}ms',
        name: 'PerformanceTestSuite',
      );
      developer.log(
        '  Total operations: ${scrollTimes.length}',
        name: 'PerformanceTestSuite',
      );
    }
  ...
  void _generateTestRecommendations() {
    developer.log(
      '💡 [PERF TEST] Test Recommendations:',
      name: 'PerformanceTestSuite',
    );
    ...
    if (_frameCount > 0) {
      ...
      if (dropRate > 10) {
        developer.log(
          '  - High frame drop rate detected (${dropRate.toStringAsFixed(1)}%) - optimize expensive operations',
          name: 'PerformanceTestSuite',
        );
      }
      if (dropRate > 20) {
        developer.log(
          '  - Critical frame drop rate - investigate main thread blocking immediately',
          name: 'PerformanceTestSuite',
        );
      }
    }
    ...
    if (_testResults['stress_test_times'] != null &&
      ...
      if (avgStressTime > 100) {
        developer.log(
          '  - Stress test operations are slow - optimize heavy computations',
          name: 'PerformanceTestSuite',
        );
      }
    }
    ...
    if (_testResults['image_load_times'] != null &&
      ...
      if (slowImages > 0) {
        developer.log(
          '  - $slowImages slow image loads detected - implement better caching/compression',
          name: 'PerformanceTestSuite',
        );
      }
    }
  ...
  void stopPerformanceTesting() {
    ...
    generateTestReport();
    developer.log(
      '🛑 [PERF TEST] Performance testing stopped',
      name: 'PerformanceTestSuite',
    );
  }
  ...
  void clearTestResults() {
    ...
    _testStartTime = null;
    developer.log(
      '🗑️ [PERF TEST] Test results cleared',
      name: 'PerformanceTestSuite',
    );
  }
}
