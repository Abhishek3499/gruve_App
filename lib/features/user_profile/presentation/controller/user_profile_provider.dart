...
class UserProfileProvider extends ChangeNotifier {
  ...
  Future<void> fetchProfile(String userId, {bool silent = false}) async {
    ...
    if (_currentUserId == userId &&
        ...
        _profile != null) {
      _log(
        '⏭️ [UserProfileProvider] Skipping duplicate fetch for userId: $userId',
      );
      return;
    }
    ...
    if (inFlight != null) {
      _log(
        '⏳ [UserProfileProvider] Joining in-flight fetch for userId: $userId',
      );
      return inFlight;
    }
  ...
  Future<void> _runFetchProfile(String userId, {required bool silent}) async {
    ...
    try {
      ...
      if (_currentUserId != userId) {
        _log(
          '⏭️ [UserProfileProvider] Stale profile response ignored: $userId',
        );
        return;
      }
      ...
      _log(
        '✅ [UserProfileProvider] Profile loaded successfully: ${userProfile.username}',
      );
    } catch (e) {
