...
class CacheInvalidationService {
  ...
  Future<void> onPostCreated(String postId) async {
    AppLogger.d('📝 [CacheInvalidation] Post created: $postId');
  ...
  Future<void> onPostLiked(String postId) async {
    AppLogger.d('❤️ [CacheInvalidation] Post liked: $postId');
    await _helper.invalidateOnAction(
  ...
  Future<void> onUserFollowed(String userId) async {
    AppLogger.d('👥 [CacheInvalidation] User followed: $userId');
    await _helper.invalidateOnAction(
  ...
  Future<void> onCommentAdded(String postId) async {
    AppLogger.d('💬 [CacheInvalidation] Comment added: $postId');
    await _helper.invalidateOnAction(
  ...
  Future<void> onMessageSent(String conversationId) async {
    AppLogger.d('📤 [CacheInvalidation] Message sent: $conversationId');
    await _helper.invalidateOnAction(
  ...
  Future<void> onProfileUpdated(String userId) async {
    AppLogger.d('👤 [CacheInvalidation] Profile updated: $userId');
    await _helper.invalidateOnAction(
  ...
  Future<void> onStoryCreated(String userId) async {
    AppLogger.d('📖 [CacheInvalidation] Story created: $userId');
    await _helper.invalidateOnAction(
  ...
  Future<void> invalidateAll() async {
    AppLogger.d('🧹 [CacheInvalidation] Invalidating all caches');
