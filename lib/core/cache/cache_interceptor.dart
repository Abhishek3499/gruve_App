...
class RequestDeduplicator {
  ...
  Future<T> deduplicate<T>(
    ...
    try {
      ...
      AppLogger.d(
        '✅ [RequestDeduplicator] Request completed: $key (${DateTime.now().difference(_requestTimestamps[key]!).inMilliseconds}ms)',
      );
      return result;
    } catch (e) {
...
class CacheInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    AppLogger.d(
      '🎯 [CacheInterceptor] 🔍 Checking cache for ${options.path} (key: $cacheKey)',
    );
    ...
    if (cacheResult.hasData && !cacheResult.isStale) {
      AppLogger.d(
        '🎯 [CacheInterceptor] ✅ Cache hit: ${options.path} (type: ${cacheResult.data!.dataType})',
      );
    ...
    if (cacheResult.hasData && cacheResult.isStale) {
      AppLogger.d(
        '🔄 [CacheInterceptor] ⏰ Stale data returned: ${options.path} (type: ${cacheResult.data!.dataType})',
      );
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    if (response.requestOptions.method.toUpperCase() == 'GET' &&
      ...
      AppLogger.d(
        '💾 [CacheInterceptor] 📝 Caching response: ${response.requestOptions.path}',
      );
      ...
      AppLogger.d(
        '💾 [CacheInterceptor] 📦 Cache data type: ${cacheData.dataType} for ${response.requestOptions.path}',
      );
      ...
      AppLogger.d(
        '💾 [CacheInterceptor] ✅ Cached response: ${response.requestOptions.path} (${cacheData.dataType})',
      );
    }
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    if (_shouldReturnStaleOnError(err)) {
      AppLogger.d(
        '🔄 [CacheInterceptor] 🔍 Attempting to return stale data on error: ${err.requestOptions.path}',
      );
      ...
      if (cached != null) {
        AppLogger.d(
          '🔄 [CacheInterceptor] ✅ Returned stale data on error: ${err.requestOptions.path} (type: ${cached.dataType})',
        );
        ...
        AppLogger.d(
          '🚫 [CacheInterceptor] ❌ No stale data available for: ${err.requestOptions.path}',
        );
      }
    }
  ...
  Future<CacheData> _performRequest(RequestOptions options) async {
    ...
    return await _deduplicator.deduplicate<CacheData>(requestKey, () async {
      AppLogger.d(
        '🚀 [CacheInterceptor] 🔍 Performing fresh request: ${options.path}',
      );
      ...
      try {
        ...
        SafeParsingHelpers.logResponseInfo(
          response.data,
          '🌐 CacheInterceptor._performRequest',
        );

        final cacheData = _createCacheData(response.data);
        AppLogger.d(
          '✅ [CacheInterceptor] 🎉 Fresh request successful: ${options.path} (${cacheData.dataType})',
        );
        return cacheData;
      } catch (e) {
        AppLogger.d(
          '💥 [CacheInterceptor] ❌ _performRequest failed for ${options.path}: $e',
        );
        rethrow;
      }
    });
  ...
  CacheData _createCacheData(dynamic data) {
    ...
    if (data is List) {
      AppLogger.d(
        '📦 [CacheInterceptor] Creating list cache data (${data.length} items)',
      );
    ...
    AppLogger.d(
      '📦 [CacheInterceptor] Creating fallback empty cache data for type: ${data.runtimeType}',
    );
    return CacheData.fromEmpty();
  }
  ...
  CacheData? _safeCacheDataParse(dynamic data, {String? context}) {
    if (data == null) {
      AppLogger.d(
        '⚠️ [CacheInterceptor] ${context ?? 'Unknown'}: data is null',
      );
      return null;
    }
    ...
    if (data is Map<String, dynamic>) {
      try {
        ...
        AppLogger.d(
          '💥 [CacheInterceptor] ${context ?? 'Unknown'}: failed to parse CacheData: $e',
        );
        return null;
      }
    }
    ...
    AppLogger.d(
      '⚠️ [CacheInterceptor] ${context ?? 'Unknown'}: data is ${data.runtimeType}, expected CacheData',
    );
    return null;
  }
...
class CacheInvalidationHelper {
  ...
  Future<void> _invalidateOnFollow(String? userId) async {
    ...
    AppLogger.d(
      '🗑️ [CacheInvalidation] Invalidated caches for follow: $userId',
    );
  }
  ...
  Future<void> _invalidateOnComment(String? postId) async {
    ...
    AppLogger.d(
      '🗑️ [CacheInvalidation] Invalidated caches for comment: $postId',
    );
  }
  ...
  Future<void> _invalidateOnMessage(String? conversationId) async {
    ...
    AppLogger.d(
      '🗑️ [CacheInvalidation] Invalidated caches for message: $conversationId',
    );
  }
  ...
  Future<void> _invalidateOnProfileUpdate(String? userId) async {
    ...
    AppLogger.d(
      '🗑️ [CacheInvalidation] Invalidated caches for profile update: $userId',
    );
  }
  ...
  Future<void> _invalidateOnStoryCreate(String? userId) async {
    ...
    AppLogger.d(
      '🗑️ [CacheInvalidation] Invalidated caches for story create: $userId',
    );
  }
